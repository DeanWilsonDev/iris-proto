...
> grammar, `RenderBlockParser`) into a C++23 expression constructing an `IrisComponent` value,
...
> assume a shape for `IrisComponent` that turns out not to be sufficient once every construct in
> the grammar (§1.4–§1.6) has to actually compile down to something.

---

## Gap 1: `<Slot>`'s callable child doesn't fit anywhere on `IrisComponent`

`docs/iris_core_spec.md` §2.5 gives `IrisComponent` as:
...
`<Slot>`'s single child is a callable returning `IrisComponent` or `std::vector<IrisComponent>`
(`docs/iris_stage3_decision_slot.md`), re-invoked later by the runtime. That callable has
nowhere to go:

- Not `Children` — that holds already-*constructed* `IrisComponent` values, not an unevaluated
  callable.
- Not `Props` — `IrisPropValue` (`docs/iris_props_decision.md`) is a closed variant whose one
  ...
  something returning `IrisComponent`/`vector<IrisComponent>`.

**Decision:** add one field to `IrisComponent`, meaningful only when `Tag == Slot`:
...
// Defined after IrisComponent in the same header, once IrisComponent is a
// complete type — a variant of function<IrisComponent()> can't safely be a
// *direct* member of IrisComponent itself: unlike std::vector, the standard
// gives std::function no guarantee of working with an incomplete return type,
// so it's held behind a pointer instead (shared_ptr, since IrisComponent must
...
§1.4) — it can't tell whether the lambda the author wrote returns `IrisComponent` or
`std::vector<IrisComponent>`, so it can't pick a `IrisSlotCallable` alternative itself. That
...
alternative; a return type that's neither `IrisComponent` nor `std::vector<IrisComponent>` fails
to compile inside the helper, which is exactly the "malformed lambda → host-compiler error"
tier from §6, not a preprocessor-level check. It lives alongside `IrisComponent` itself
...
`IrisComponent` tree and drives re-invocation is what's lowercase `iris::`.
...
or literal text" — but nothing in `IrisComponent`'s shape holds a bare string or an
unwrapped expression as a child; `Children` only holds `IrisComponent` values.
...
- **Every other primitive that accepts children** (`<Inline>` for mixed inline-flow content,
  ...
  ```cpp
  IrisComponent{ IrisElementTag::Text,
  ...
  type alongside `IrisComponent`: every child of every non-`<Text>`, non-`<Slot>` primitive is,
  after codegen, an ordinary `IrisComponent` — some of them just happen to be synthesized
...
- **`key` is parsed but not threaded through codegen.** `IrisComponent` has no `Key` field
...
- **Namespace qualification in generated code.** No spec example commits to one, so codegen
  fully qualifies every IR type it emits (`Iris::IrisComponent`, `Iris::IrisElementTag::Frame`,
