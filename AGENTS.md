...
- **`include/Iris/` + `src/Iris/`** — the preprocessor front end, with two
  `IHostLanguageTokenizer` implementations: `CppTokenizer` for C++23 `.iris` source, and
  `NyxTokenizer` (wrapping nyx-proto's own `Lexer`) for Nyx `.irisx` source — selected by file
  extension at preprocessor startup. `CppTokenizer`'s job is narrow and deliberate: understand
  just enough C++ lexical syntax (identifiers, braces, string/char literals with escapes,
  line/block comments) to detect `render { }` blocks and keep `{ }` escape-hatch
  brace-balancing from desyncing on braces that merely *look* like real braces inside a
  string, char literal, or comment. It does not tokenize the full C++ grammar — numbers,
  operators, and everything else fall through as `TokenKind::Other`; validating that is the
  host C++ compiler's job on generated output, never Iris's.
- **Nyx as a host language (`.irisx`) is implemented, not deferred** — `NyxTokenizer`,
  `IrisNyxDriver`, `IrisNyxEvaluator`, `NyxComponentBridge`, `NyxLifecycleAdapter`, and
  `NyxSignalDecorator` (`include/Iris/`, `src/Iris/`) are real, tested code, and `pharos-proto`
  already builds a whole production panel tree on it (`src/ui/nyx/*.irisx`, mounted via
  `Iris::IrisNyxDriver::MountRoot`). Don't confuse this with §6's "Stage 6 (deferred): Umbra
  Engine backend" in `docs/iris_handoff.md` — that's a *different* axis (which engine renders
  the widgets: Penumbra now vs. a future Umbra Engine renderer), not whether Nyx can author
  components against the existing Penumbra backend. It can, today.
- **`IHostLanguageTokenizer`** is the abstraction point both host languages plug into — nothing
  else in the preprocessor core should carry host-language-specific lexical rules. A third host
  language would add a third concrete tokenizer here, same pattern.
...
(language spec, done), Stage 1 (this preprocessor front end, both `CppTokenizer` and
`NyxTokenizer`, done), Stage 2 (Penumbra backend, static widget tree, done — in
`penumbra-ui-backend`), Stage 3 (reactive runtime — state, reconciler, `<Slot>`-scoped
diffing, `iris::Tick()`, done), Stage 4 (Lustre styling — done and in production use, see
`penumbra-ui-backend`'s `Lustre/StyleApplier.cpp`/`StyleResolution.cpp`/`StylesheetLoader.cpp`
and the sibling `lustre` repo's own resolver), Stage 5 (first real consumer, done —
`pharos-proto`'s `src/ui/nyx/*.irisx` + `src/ui/iris/*.lustre` is a whole production panel
tree on this stack), Stage 6 (still deferred: a second, Umbra Engine rendering *backend* —
unrelated to Nyx-as-host-language above, see the Nyx bullet under Architecture). Stages 0–5
are real, tested implementation, not design docs — verify against actual source (or a
consumer like `pharos-proto`) before assuming a stage is unbuilt; only Stage 6 is still
genuinely just a plan.
